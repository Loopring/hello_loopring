...
class LoopringV3Client(RestClient):
    ...
    def sign(self, request):
        ...
        if security & Security.EDDSA_URL_SIGN:
            ...
            ecdsa_sig = "0x" + bytes.hex(v_r_s_to_signature(v, r, s)) + EthSignType.EIP_712
            headers.update({"X-API-SIG": ecdsa_sig})

        request.path = path
        if request.method not in ["GET", "DELETE"]:
            request.data = ujson.dumps(request.data) if request.data else request.params
            request.params = {}
        else:
            request.data = {}
    ...
    def on_query_amm_pools(self, data):
    ...
    @staticmethod
    def on_get_user_data(data, request):
    ...
    def get_apiKey(self):
        """"""
        data = {"security": Security.EDDSA_URL_SIGN}
    ...
    def on_query_balance(self, data):
        for balance in data:
            ...
            for token in self.tokenIds.values():
                if token == balance['tokenId']:
                    ...
                    print(f"Account balance {token_symbol}: {float(tokenAmount)/(10**decimals)},"
                          f" locked: {float(frozenAmount)/(10**decimals)}")
    ...
    def offchainWithdraw_eddsa(self, to_b, token, amount, minGas,
        ...
        data = {"security": Security.ECDSA_EIP_AUTH}
        req = self._create_offchain_withdraw_request(
            to_b,
            token,
            amount,
            feeToken,
    ...
    def _create_offchain_withdraw_request(
        self,
        to: str,
        token,
        amount: float,
    ...
    @staticmethod
    def on_send_order(data, request):
    ...
    @staticmethod
    def on_cancel_order(data, request):
        """"""
